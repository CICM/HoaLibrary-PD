...
extern t_hoa_err hoa_getinfos(t_hoa_meter* x, t_hoa_boxinfos* boxinfos)
{
...
extern t_pd_err offset_get(t_hoa_meter *x, void *attr, long *argc, t_atom **argv)
{
    ...
    if(argv[0] && argc[0])
    {
        atom_setfloat(argv[0], x->f_meter->getPlanewavesOffset() / HOA_2PI * 360.);
    }
    else
    {
        argc[0] = 0;
...
extern t_pd_err offset_set(t_hoa_meter *x, void *attr, long argc, t_atom *argv)
{
    ...
    {
        x->f_vector->setPlanewavesOffset(atom_getfloat(argv) / 360 * HOA_2PI);
        x->f_meter->setPlanewavesOffset(atom_getfloat(argv) / 360 * HOA_2PI);
        x->f_vector->computeRendering();
        x->f_meter->computeDisplay();
    }
