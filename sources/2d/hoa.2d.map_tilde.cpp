...
typedef struct _hoa_map_tilde
{
    ...
    hoa::EncoderMulti<hoa::Hoa2d, t_sample>*    f_map;
...
static t_class *hoa_map_tilde_class;
static t_symbol* hoa_sym_car;
static t_symbol* hoa_sym_cartesian;
static t_symbol* hoa_sym_polar;

static void *hoa_map_tilde_new(t_symbol *s, int argc, t_atom *argv)
{
    t_hoa_map_tilde *x  = (t_hoa_map_tilde *)pd_new(hoa_map_tilde_class);
    if(x)
	{
		const size_t order = hoa_processor_clip_order(x, (size_t)atom_getfloatarg(0, argc, argv));
        const size_t nsrcs = hoa_processor_clip_order(x, (size_t)atom_getfloatarg(1, argc, argv));
        const t_symbol* m  = atom_getsymbolarg(2, argc, argv);
        x->f_mode = (m == hoa_sym_car || m == hoa_sym_cartesian) ? hoa_sym_cartesian : hoa_sym_polar;

        x->f_ramp       = 100;
		x->f_map        = new hoa::EncoderMulti<hoa::Hoa2d, t_sample>(order, nsrcs);
		x->f_lines      = new hoa::PolarLines<hoa::Hoa2d, t_sample>(x->f_map->getNumberOfSources());
        x->f_lines->setRamp(0.1 * sys_getsr());

        for(size_t i = 0; i < x->f_map->getNumberOfSources(); i++)
        {
            x->f_lines->setRadiusDirect(i, 1);
            x->f_lines->setAzimuthDirect(i, 0.);
        }
        const size_t nins = (x->f_map->getNumberOfSources() == 1) ? 3 : x->f_map->getNumberOfSources();
        hoa_processor_init(x, nins, x->f_map->getNumberOfHarmonics());
        x->f_ins            = new t_sample[nins * 81092];
        x->f_outs           = new t_sample[x->f_map->getNumberOfHarmonics() * 81092];
